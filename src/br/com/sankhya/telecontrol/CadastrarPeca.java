...
public class CadastrarPeca implements AcaoRotinaJava {

	@Override
	public void doAction(ContextoAcao ctx) throws Exception {
		try {
			Integer codPeca = (Integer) ctx.getParam("CODPROD");
			BigDecimal codprodPeca = new BigDecimal(codPeca);

			apiPostPecas("http://api2.telecontrol.com.br/posvenda-core/peca", ctx, codprodPeca);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void apiPostPecas(String ur, ContextoAcao ctx, BigDecimal codprodPeca) throws Exception {
		...
		while (rs.next()) {
			...
			if (responseCode == 201) {
				...
				update(ctx, codProd, "TGFPRO", "AD_ENVIADOTELECONTROL", "CODPROD");
			}
	...
	public static void update(ContextoAcao ctx, int codpk, String tabela, String campo, String cond) {
		...
		try {
			...
			pstmt.executeUpdate();
			ctx.setMensagemRetorno("Peça Cadastrada no Telecontrol");
