...
public class InserePedidoSankhya implements AcaoRotinaJava {
	...
	public void doAction(ContextoAcao ctx) throws Exception {
		...
		try {
		Timestamp dataIni = (Timestamp) ctx.getParam("DATAINI");
		Timestamp dataFin = (Timestamp) ctx.getParam("DATAFIN");

		// Convertendo para LocalDate
		LocalDate localDateIni = dataIni.toLocalDateTime().toLocalDate();
		LocalDate localDateFin = dataFin.toLocalDateTime().toLocalDate();

		// Formatando as datas como strings
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		String dataIniFormatada = localDateIni.format(formatter);
		String dataFinFormatada = localDateFin.format(formatter);

		// Usando as datas formatadas
		
		apiFaturamento(ctx, dataIniFormatada, dataFinFormatada);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
public void apiFaturamento(ContextoAcao ctx, String dataIniFormatada,  String dataFinFormatada ) throws Exception {
		...
		String[] result = api("http://api2.telecontrol.com.br/posvenda-pedido/pedidos/dataInicial/"+dataIniFormatada+"/dataFinal/"+dataFinFormatada);
		//String[] result = api("http://api2.telecontrol.com.br/posvenda-pedido/pedidos/dataInicial/2024-06-18/dataFinal/2024-06-18");
	    ...
	    try{
		    ...
		    for (int i = 0; i < pedidos.size(); i++) {
		        ...
		        System.out.println("empresa: " + codEmp + " top: " + top + " dhtop: " + dhTop
		        		+ " tipmov: " + tipMov + " codparc: " + codparc + " codTipNeg: " + codTipNeg
		        		+ " dhtipneg: " + dhTipNeg + " codcecus: " + codcecus + " natureza: " + natureza
		        		+ " qtdVol: " + qtdVol + " pesliq: " + pesoLiq + " pesobruto: " + pesoBruto + " Entrega: " + adEntrega);
...
//		        
		        try{
			        ...
			        for (int j = 0; j < itens.size(); j++) {
						...
						if(verificarEstoque(codlocal, new BigDecimal(codEmp), codprod)){
								System.out.println("nunota: " + nunota + " codEmp: " + codEmp + " codprod: " + 
							codprod + " quantidade: "  + qtdNeg + " vlrUnit: " + vlrUnit + " vlrTotal: " + vlrTotal + " usoProd: " + 
										usoProd + " codVol: " + codVol + " codlocal: " + codlocal);
								
								insertIte(ctx, new BigDecimal(nunota), new BigDecimal(codEmp), codprod,
				            			qtdNeg, vlrUnit, vlrTotal, usoProd, codVol, codlocal, pedidoItem, ipi);
								
								System.out.println("Inseriu");
								ctx.setMensagemRetorno("Inseriu " + "nunota: " + nunota + " codEmp: " + codEmp + " codprod: " + 
							codprod + " quantidade: "  + qtdNeg + " vlrUnit: " + vlrUnit + " vlrTotal: " + vlrTotal + " usoProd: " + 
										usoProd + " codVol: " + codVol + " codlocal: " + codlocal + " Entrega: " + adEntrega);
						
						}else{
							ctx.setMensagemRetorno("Sem estoque para o produto: " + codprod + " no local: " + codlocal);
	...
	public void inserirPar(ScheduledActionContext contexto) throws Exception {
		...
		jdbc.openSession();
		String[] response = api("https://private-anon-fe98d2f5d6-posvendamestre.apiary-mock.com/posvenda-core/revendaFabrica");
...
//		pstmt.setBigDecimal(3, codprod);
	...
	public boolean insertCab(BigDecimal nunota, BigDecimal tipOper, String tipMov, String dhTipOper, 
			BigDecimal tipVenda, String dhTipVenda, BigDecimal codCenCus, BigDecimal codEmp, 
			BigDecimal codparc, BigDecimal natureza, BigDecimal qtdVol, BigDecimal pesoLiq, 
			BigDecimal pesoBruto) throws Exception{
		...
		try{
				...
				String sqlUpdate = "INSERT INTO TGFCAB(NUNOTA, "
						+ "NUMNOTA, "
						+ "DTNEG, "
						+ "CODTIPOPER, "
						+ "TIPMOV, "
	...
	public void insertIte(ContextoAcao ctx, BigDecimal nunota, BigDecimal codemp, BigDecimal codprod,
			BigDecimal qtdNeg, BigDecimal vlrUnit, BigDecimal vlrTotal, 
			String usoprod, String codvol, BigDecimal codlocal, String pedidoItem, BigDecimal ipi) throws Exception{
		EntityFacade entityFacade = EntityFacadeFactory.getDWFFacade();
		JdbcWrapper jdbc = entityFacade.getJdbcWrapper();
		PreparedStatement pstmt = null;
		...
		jdbc.openSession();
						...
						String sqlUpdate = "INSERT INTO TGFITE(NUNOTA, "
								+ "SEQUENCIA, "
								+ "CODEMP, "
								+ "CODPROD, "
								+ "QTDNEG, "
	...
	public BigDecimal getVlrNota(BigDecimal nunota) throws Exception {
		...
		try {
			...
			String sqlNota = "SELECT FORMAT((SUBQUERY.VLRTOTAL + SUBQUERY.VLRIPI), '0.00') AS TOTAL "
						+"	FROM (SELECT SUM(VLRTOT) AS VLRTOTAL, SUM(VLRIPI) AS VLRIPI "
						+"			FROM TGFITE "
	...
	public void insertInfoPedido(BigDecimal nunota) throws Exception{
		...
		String sqlUpdate = "INSERT INTO AD_INFOCABTELECONTROL "
				+ " (ID, NUNOTA, FATURADO) "
